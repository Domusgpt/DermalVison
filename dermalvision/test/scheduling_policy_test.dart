...
void main() {
  ...
  group('scheduling helpers', () {
    ...
    test('changedOrNewReminders returns only new/changed schedules', () {
      final next = [
        ScheduledReminder(
          id: 1,
          title: 'a',
      ...
      final changed = changedOrNewReminders(
        {
          1: DateTime(2026, 2, 18, 10, 0),
          2: DateTime(2026, 2, 18, 10, 30),
        },
        next,
      );

      expect(changed.map((e) => e.id), [2]);
    });

    test('reminderScheduleById maps reminder ids to dates', () {
      final reminders = [
        ScheduledReminder(
          id: 7,
          title: 'x',
          body: 'x',
          scheduledDate: DateTime(2026, 2, 19, 9, 0),
        ),
      ];

      final map = reminderScheduleById(reminders);
      expect(map[7], DateTime(2026, 2, 19, 9, 0));
    });
