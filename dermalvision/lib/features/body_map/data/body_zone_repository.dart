...
class FirebaseBodyZoneRepository implements BodyZoneRepository {
  ...
  Stream<List<BodyZone>> watchZones(String uid) {
    return _firestore.collection('users').doc(uid).collection('zones').snapshots().map((snapshot) {
  ...
  Future<void> addZone(String uid, BodyZone zone) {
    return _firestore.collection('users').doc(uid).collection('zones').doc(zone.id).set(zone.toJson());
  }

  @override
  Future<void> updateZone(String uid, BodyZone zone) {
    return _firestore.collection('users').doc(uid).collection('zones').doc(zone.id).update(zone.toJson());
  }

  @override
  Future<void> deleteZone(String uid, String zoneId) {
    return _firestore.collection('users').doc(uid).collection('zones').doc(zoneId).delete();
  }

  @override
  Future<BodyZone?> getZone(String uid, String zoneId) async {
    final doc = await _firestore.collection('users').doc(uid).collection('zones').doc(zoneId).get();
