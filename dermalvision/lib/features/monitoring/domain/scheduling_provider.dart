...
void schedulingManager(SchedulingManagerRef ref) {
  ...
  final lastScheduleById = <int, DateTime>{};
  ...
  ref.listen(bodyZonesProvider, (previous, next) {
    next.whenData((zones) {
      pendingSync = pendingSync.then((_) async {
        ...
        final idsToCancel = staleReminderIds(lastScheduledIds, reminders);
        final remindersToSchedule =
            changedOrNewReminders(lastScheduleById, reminders);
        ...
        lastScheduledIds
          ..clear()
          ..addAll(reminders.map((e) => e.id));

        lastScheduleById
          ..clear()
          ..addAll(reminderScheduleById(reminders));
      });
    });
  });
}
