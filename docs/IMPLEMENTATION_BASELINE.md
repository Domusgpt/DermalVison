...
7. **Repository hygiene audit** (`tools/bootstrap/audit_repo_hygiene.sh`)
   - Fails fast if disallowed binary artifacts are tracked in git.
   - Acts as a guardrail for cloud-terminal compatibility before PR submission.
...
# Ensure disallowed binaries are not tracked
./tools/bootstrap/audit_repo_hygiene.sh
